...
static __always_inline unsigned long *__page_mapcount_seq(struct page *page)
{
	/*
	 ...
	 *
	 * The writer is serialized by the &page[2].flags PG_lock bit
	 * spinlock (page_mapcount_lock/page_mapcount_unlock).
	 */
	return &page[2].mapcount_seqcount;
}
...
static inline void page_mapcount_lock(struct page *page)
{
	...
	bit_spin_lock(PG_locked, &page[2].flags);
...
static inline void page_mapcount_unlock(struct page *page)
{
	smp_wmb();
	*__page_mapcount_seq(page) += 1;
	bit_spin_unlock(PG_locked, &page[2].flags);
}
