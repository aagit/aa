					 ...
					 struct anon_vma *anon_vma)
{
	...
	if (page->index == linear_page_index(vma, address)) {
		...
		if (!page_mapped(page)) {
			 ...
			 *
			 * The swap entry of THP swapcache cannot be a
			 * KSM generated swapentry, so the linearity
			 * is guaranteed and the anon_vma lifetime as
			 * well.
			 */
			if (!PageTransCompound(page)) {
				page->mapping = NULL;
				...
				VM_WARN_ON_ONCE_PAGE(anon_vma->root !=
						     vma->anon_vma->root,
						     page);
			}
			return false;
		...
		/* THP swapcache is always linearly mapped */
		VM_WARN_ON_ONCE_PAGE(PageTransCompound(page), page);
	}
	return true;
}
