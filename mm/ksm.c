			...
			struct vm_area_struct *vma, unsigned long address)
{
	...
	if (PageKsm(page)) {
		...
		if (!page_mapped(page)) {
			 ...
			 * been freed (it may not even be part of the
			 * anon_vma_chain of the current
			 * vma->anon_vma).
			 *
			 ...
			 *    as the rcu_read_lock is being hold.
			 *    It's ok if the page is unmapped from
			 *    under us while we do the check, because
			 *    if pass the check the it means even if
			 *    the page was unmapped, the anon_vma the
			 *    page points to is linear with with the
