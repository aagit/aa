 ...
 *
 * This also means FOLL_LONGTERM users can stop issuing
 * FOLL_FORCE|FOLL_WRITE to take readonly page pins because that was a
 * band aid required due the lack of un-sharing integrated in GUP.
 */
...
static inline vm_fault_t wp_huge_pmd(struct vm_fault *vmf, pmd_t orig_pmd)
{
	if (vmf->flags & FAULT_FLAG_UNSHARE) {
		if (is_cow_mapping(vmf->vma->vm_flags))
			goto fallback;
		else
			return 0;
	}
