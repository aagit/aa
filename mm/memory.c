 ...
 *
 * FOLL_LONGTERM pins (unlike short term pins) to remain fully
 * coherent need to unshare PageKsm, MAP_PRIVATE pagecache and
 * zeropages. Short term pins don't need to unshare those readonly
 * page types because short term pins will refresh the physaddr from
 * the pgtable.
 */
		...
		if (page_count(vmf->page) == 1) {
			...
			if (trylock_page(vmf->page)) {
				...
				if (reused) {
					 ...
					 * pgtable is needed.
					 */
					VM_WARN_ON(pte_write(vmf->orig_pte));
					goto out_unlock;
				}
			}
		}
...
static inline vm_fault_t wp_huge_pmd(struct vm_fault *vmf, pmd_t orig_pmd)
{
	...
	VM_WARN_ON(!(vmf->flags & FAULT_FLAG_WRITE));
	if (vma_is_anonymous(vmf->vma)) {
