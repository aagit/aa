...
static void page_remove_anon_compound_rmap(struct page *page)
{
	...
	if (PageDoubleMap(page)) {
		 ...
		 *
		 * Without the page_mapcount_lock such jittering could
		 * happen and FOLL_LONGTERM could take the GUP pin in
		 * b) without FOLL_UNSHARE COR, and another thread in
		 * c) could trigger a spurious COW immediately later,
		 * causing the readonly long term GUP pin to lose
		 * coherency.
		 */
